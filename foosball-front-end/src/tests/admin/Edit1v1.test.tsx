...
test('swap button swaps users', async () => {
    ...
    expect(playerNames[0].value).toBe(`${username}1`);
    expect(playerNames[1].value).toBe(`${username}2`);
    fireEvent.click(screen.getByText(/Swap/i));
    const playerNamesAfterSwap = screen.getAllByPlaceholderText(/Username/i) as HTMLInputElement[];
    expect(playerNamesAfterSwap[0].value).toBe(`${username}2`);
    expect(playerNamesAfterSwap[1].value).toBe(`${username}1`);
});
