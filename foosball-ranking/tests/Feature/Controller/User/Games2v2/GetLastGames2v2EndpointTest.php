...
class GetLastGames2v2EndpointTest extends TestCase
{
    use RefreshDatabase;

    public function test_user_can_see_the_last_2v2_game_played_when_there_are_less_then_10(): void
    {
        ...
        //We can't use directly the game in the database because the endpoint returns the name using a joi
        //As such we create the game manually
        $expectedGame = self::createExpectedGame($team1, $team2, 10, 5, 1);
        $this->assertEquals($expectedGame, self::get2v2Games($players[0], 1)[0]);
    }
    ...
    private function get2v2Games($player, $page)
    {
        $this->post('/login', [
            'email' => $player->email,
            'password' => 'password',
        ...
        $games = array();
        $response = $this->get('/games2v2?page=' . $page)->assertStatus(200);
    ...
    public function test_user_can_see_only_10_2v2games_per_page(): void
    {
        $players = self::create_players(4);
        $team1 = self::createTeam($players[0], $players[1], 'team1');
        $team2 = self::createTeam($players[2], $players[3], 'team2');
        ...
        $expectedGame = self::createExpectedGame($team1, $team2, 10, 5, 1);
        $games = self::get2v2Games($players[0], 1);
        for ($i = 0; $i < 10; $i++) {
    ...
    public function test_user_can_see_the_games_even_if_not_created_by_them(): void
    {
        ...
        $expectedGame = self::createExpectedGame($team1, $team2, 10, 5, 1);
        $games = self::get2v2Games($players[4], 1);
        for ($i = 0; $i < 10; $i++) {
            $this->assertEquals($expectedGame, $games[$i]);
        }
    }

    public function test_user_cant_see_the_last_2v2_games_when_not_logged_in(): void
    {
        $players = self::create_players(4);
        $team1 = self::createTeam($players[0], $players[1], 'team1');
        $team2 = self::createTeam($players[2], $players[3], 'team2');
        $result = self::create2v2Game($players[0], $players[1], $players[2], $players[3], 10, 5, 1);
        $this->assertNotNull($result[1]);
        $this->json('get', '/games2v2')->assertStatus(401);
    }

    public function test_returns_empty_if_no_2v2_games_are(): void
    {
        $players = self::create_players(1);
        $this->assertEmpty(self::get2v2Games($players[0], 1));
    }

    public function test_2v2_games_are_ordered_descengly_by_date(): void
    {
        $players = self::create_players(4);
        $team1 = self::createTeam($players[0], $players[1], 'team1');
        $team2 = self::createTeam($players[2], $players[3], 'team2');
    ...
    public function test_2v2_games_are_paginated_with_10_games_per_page()
    {
        $players = self::create_players(4);
        $team1 = self::createTeam($players[0], $players[1], 'team1');
        $team2 = self::createTeam($players[2], $players[3], 'team2');
        ...
        $games = self::get2v2Games($players[0], 2);
        for ($i = 0; $i < 3; $i++) {
            $this->assertEquals($expectedGame, $games[$i]);
        }
    }
